...
\beq
\langle \Phi_I | \op{H} | \Phi^J \rangle = \sgn_{ia} \left[ h_{ia} 
    + \sum_{j \in I} \left[ v_{ij}^{aj} - v_{ij}^{ja} \right] \right]
\eeq
...
\beq
\langle \Phi_I | \op{H} | \Phi^J \rangle = \sgn_{iajb} \left[ v_{ij}^{ab} - v_{ij}^{ba} \right]
\eeq
...
For each component, string substitutions are performed:
\begin{itemize}
\item \textbf{One-electron terms}: Substitution in one string, sum over other string
...
\left(\mat{H}_{\{P\}} - E^\st{n}\right)\vect{x}^\st{n}_{\{P\}}
  \left(\vect{x}_{\st{n}\{Q\}}^\dagger \vect{u}_{\{Q\}}\right) + \beta \vect{x}^\st{n}_{\{P\}} \\
...
\textbf{Phase III} (Selection): Generate candidate determinants connected to the variational 
space by $|H_{J}^I c_I| \gt \epsilon_1$:


For each determinant $|\Phi_I\rangle$ in the variational space:
\begin{itemize}
\item Generate single excitations with $|f_{i}^{a} c_I| \gt \epsilon_1$
\item Use pre-sorted lists to generate only double excitations with $|v_{ij}^{ab} c_I| \gt \epsilon_{\text{adaptive}}$
\item Adaptive threshold: $\epsilon_{\text{adaptive}} = \epsilon_1 / |c_I|$
\end{itemize}

\textbf{Phase IV} (extension of HCI towards CIPSI): Select determinants with highest selection probabilities:
\beq
P_J = \sum_{I \in \text{var}} \frac{|H^I_{J} c_I|^2}{(E_{\text{var}} - H^J_{J})^2}
\eeq
...
\beq
P_J = \max_{\st{n}=\st{1},\ldots,\st{N}_{\text{states}}} \left[ \sum_{I \in \text{var}} 
    \frac{|H^I_{J} c^\st{n}_{I}|^2}{(E^\st{n}_{\text{var}} - H^J_{J})^2} \right]
\eeq
...
\beq
N_{\text{small}} = \max\left(100, \frac{N_{\text{target}}}{10}, 5 N_{\text{states}}\right)
\eeq

Select determinants using hybrid method (energy + excitation level), diagonalize to get 
initial eigenvectors for all states. This prevents missing excited states due to poor 
initialization.
...
\beq
\epsilon_{\text{adaptive}} = \frac{\epsilon_2}{|c_I|},
\eeq
and only excitations are generated with $|H_{IK}| \gt \epsilon_{\text{adaptive}}$ using pre-sorted lists from setup. Accumulate contributions:
