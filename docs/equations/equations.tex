...
The $\alpha$ and $\beta$ spin orbitals are denoted by $\spa{p}$ and $\spb{p}$. For RHF with positron, we 
use $\spa{p}$ for the electron orbitals and $\spb{p}$ for the positron orbitals.
...
The transformed density-fitted integrals which are used throughout \elemcoil
...
With positrons, the corrections to the one-body term of the electron Hamiltonian are given by
\begin{equation}
  \begin{aligned}
    \tilde h_p^q &= \tilde f_p^q 
    - \sum_L \left(2v_{p}^{qL} v_{i}^{iL} - v_{p}^{iL} v_{i}^{qL} - v_{p}^{qL} v_{\bar{i}}^{\bar{i}L}
    \right)\\
    \tilde{f}_p^q &= h_p^q + \sum_{\tilde{L}} \left(2v_{p}^{q\tilde L} v_{\tilde i}^{\tilde i\tilde L} 
    - v_{p}^{\tilde i\tilde L} v_{\tilde i}^{q\tilde L} - v_{p}^{q\tilde L} v_{\bar{i}}^{\bar{i}\tilde L} \right),\\
  \end{aligned}
\end{equation}
where $\bar{i}=1$ denote the single occupied positron orbital.The positron one-body terms get corrections according to 
\begin{equation}
  \begin{aligned}
    \tilde h_{\spb p}^{\spb q (\mathbf{+})} &= \tilde f_{\spb p}^{\spb q (\mathbf{+})} 
    + \sum_L \left(2v_{i}^{iL}v_{\spb p}^{\spb qL}\right)\\
    \tilde f_{\spb p }^{\spb q (\mathbf{+})} &= h_{\spb p}^{\spb q (\mathbf{+})} - \sum_L \left(2v_{ \tilde i}^{ \tilde i \tilde L}v_{\spb p}^{\spb q \tilde L}\right), 
    \\
  \end{aligned}
\end{equation}
and the energy is saved as
\begin{equation}
\tilde{E}_0 = E_0 + h_{i}^{i} + \tfrac12 h^{\spb i(\mathbf{+})}_{\spb i} - \left( \tilde h_{ i}^{ i} + \tfrac12 \tilde h^{\spb i(\mathbf{+})}_{\spb i}  \right).
\end{equation}
...
We keep the convention of barred indices $\spb{p},\spb{q},\ldots$ for
positron spin--orbitals and ordinary Latin indices $p,q,\ldots$ for the
electrons.
...
\begin{equation}
E_{\text{HF}}^{e/e^+} = \tfrac12 D_{\mu}^{\nu} \bigl(h_{\nu}^{\mu} + f_{\nu}^{\mu(e)} + J_{\mu}^{\nu}[P]\bigr)
+        P_{\mu}^{\nu}  f_{\nu}^{\mu(p)}
+ E_0,
\end{equation}
which reduces to the ordinary closed--shell expression when
$P\to0$.  In practice the Coulomb terms are accumulated on the fly
...
E^{(2)} = \frac{1}{4} \sum_{ijab} \frac{|\langle ij \| ab \rangle|^2}
{\varepsilon_i + \varepsilon_j - \varepsilon_a - \varepsilon_b},
\end{equation}
where $i,j,a,b$ denote \emph{spin-orbitals}, and the antisymmetrized integral is
\begin{equation}
...
The integral $\langle i_{\sigma_1} j_{\sigma_2} | a_{\sigma_3} b_{\sigma_4} \rangle$
...
\begin{equation}
\langle i_\alpha j_\alpha \| a_\alpha b_\alpha \rangle 
= \langle ij | ab \rangle - \langle ij | ba \rangle.
\end{equation}

\paragraph{Opposite-spin ($\alpha\beta$ and $\beta\alpha$).}
The exchange term vanishes by spin orthogonality:
\begin{equation}
\langle i_\alpha j_\beta \| a_\alpha b_\beta \rangle = \langle ij | ab \rangle.
\end{equation}
...
= 2 \times \frac{1}{4} \sum_{ijab} \frac{(\langle ij|ab\rangle - \langle ij|ba\rangle)^2}{\Delta_{ijab}}.
\end{equation}
...
= 2 \times \frac{1}{2} \sum_{ijab} \frac{\langle ij|ab\rangle^2}{\Delta_{ijab}}.
\end{equation}

\subsection{Expanding the Same-Spin Term}
Expand the squared antisymmetric integral:
\begin{equation}
(\langle ij|ab\rangle - \langle ij|ba\rangle)^2 
= \langle ij|ab\rangle^2 - 2\langle ij|ab\rangle\langle ij|ba\rangle + \langle ij|ba\rangle^2.
\end{equation}
Since $\sum_{ijab} \langle ij|ba\rangle^2 = \sum_{ijab} \langle ij|ab\rangle^2$ 
by relabeling $a \leftrightarrow b$:
\begin{equation}
\sum_{ijab} (\langle ij|ab\rangle - \langle ij|ba\rangle)^2 
= 2 \sum_{ijab} \bigl(\langle ij|ab\rangle^2 - \langle ij|ab\rangle\langle ij|ba\rangle\bigr).
\end{equation}
...
E^{(2)}_{\mathrm{SS}} = \frac{1}{2} \cdot 2 \sum_{ijab} 
  \frac{\langle ij|ab\rangle^2 - \langle ij|ab\rangle\langle ij|ba\rangle}{\Delta_{ijab}}
= \sum_{ijab} \frac{\langle ij|ab\rangle(\langle ij|ab\rangle - \langle ij|ba\rangle)}{\Delta_{ijab}}.
\end{equation}
The opposite-spin contribution simplifies to:
\begin{equation}
E^{(2)}_{\mathrm{OS}} = \sum_{ijab} \frac{\langle ij|ab\rangle^2}{\Delta_{ijab}}.
\end{equation}
...
&= \sum_{ijab} \frac{\langle ij|ab\rangle^2 - \langle ij|ab\rangle\langle ij|ba\rangle 
   + \langle ij|ab\rangle^2}{\Delta_{ijab}} \\[4pt]
&= \sum_{ijab} \frac{2\langle ij|ab\rangle^2 - \langle ij|ab\rangle\langle ij|ba\rangle}{\Delta_{ijab}}.
\end{aligned}
\end{equation}
Factoring:
\begin{equation}
...
Using spatial-orbital amplitudes $T_{ij}^{ab} = \langle ij|ab\rangle / \Delta_{ijab}$:
\begin{equation}
\begin{aligned}
E^{(2)}_{\mathrm{SS}} &= \sum_{ijab} \langle ij|ab\rangle (T_{ij}^{ab} - T_{ij}^{ba}), \\
E^{(2)}_{\mathrm{OS}} &= \sum_{ijab} \langle ij|ab\rangle \, T_{ij}^{ab}, \\
E^{(2)}_{\mathrm{RHF}} &= \sum_{ijab} \langle ij|ab\rangle (2T_{ij}^{ab} - T_{ij}^{ba}).
\end{aligned}
\end{equation}


\section{RHF--MP2 with a Single Positron}
\label{sec:rhf-positron-mp2}

Consider a closed-shell RHF system with an additional single positron.
We use indices $i,j,a,b$ for electrons and $p,q,r,s$ for positrons, where
$i,j,p$ denote occupied and $a,b,q$ denote virtual orbitals.
...
The electron--positron integral $\langle ip|aq\rangle$ describes 
electron~1 in orbital~$i$ and positron~2 in orbital~$p$ scattering to 
virtual orbitals $a$ and $q$. The attractive electron--positron interaction 
has the opposite sign to electron--electron repulsion; we assume this sign 
is already absorbed into the integral definition.

Since electrons and positrons are distinguishable particles, there is 
\emph{no exchange} between them---only the direct (Coulomb) term contributes.
The spin-orbital expression is:
\begin{equation}
E^{(2)}_{\mathrm{ep}} = \sum_{ipaq} 
\frac{|\langle i_\sigma p_\tau | a_\sigma q_\tau \rangle|^2}
{\varepsilon_i + \varepsilon_p - \varepsilon_a - \varepsilon_q},
\end{equation}
where spin is conserved independently for each particle type 
($\sigma_i = \sigma_a$ and $\tau_p = \tau_q$).

\subsection{Spin Integration}
For RHF electrons, the $\alpha$ and $\beta$ spatial orbitals are identical.
The single positron occupies one spatial orbital $p$ with a definite spin 
(say $\alpha$). There are two contributing spin configurations:
\begin{itemize}
\item $\alpha$-electron with $\alpha$-positron: $e_\alpha p_\alpha \to a_\alpha q_\alpha$
\item $\beta$-electron with $\alpha$-positron: $e_\beta p_\alpha \to a_\beta q_\alpha$
\end{itemize}
Both give the same spatial integral $\langle ip|aq\rangle$. Summing over 
electron spin:
\begin{equation}
E^{(2)}_{\mathrm{ep}} = 2 \sum_{ipaq} 
\frac{\langle ip|aq\rangle^2}{\Delta_{ipaq}},
\end{equation}
where $\Delta_{ipaq} = \varepsilon_i + \varepsilon_p - \varepsilon_a - \varepsilon_q$,
with $i,a$ running over spatial electron orbitals and $p,q$ over spatial 
positron orbitals (one occupied, several virtual).

\subsection{Amplitude Form}
Defining the electron--positron amplitude:
\begin{equation}
T_{ip}^{aq} = \frac{\langle ip|aq\rangle}{\Delta_{ipaq}},
\end{equation}
the electron--positron correlation energy becomes:
\begin{equation}
\label{eq:ep-mp2}
\boxed{E^{(2)}_{\mathrm{ep}} = 2 \sum_{ipaq} \langle ip|aq\rangle \, T_{ip}^{aq}.}
\end{equation}
...
with different orbital energies. We use lowercase $(i,j,a,b)$ for $\alpha$ 
and uppercase $(I,J,A,B)$ for $\beta$.
...
\begin{equation}
E^{(2)}_{\alpha\alpha} = \frac{1}{4} \sum_{ijab}
\frac{(\langle ij|ab\rangle - \langle ij|ba\rangle)^2}{\Delta_{ijab}^\alpha}
= \frac{1}{2} \sum_{ijab} \langle ij|ab\rangle \, T_{ij}^{ab,\alpha\alpha}.
\end{equation}
The $\tfrac{1}{4} \to \tfrac{1}{2}$ follows from amplitude antisymmetry.

\paragraph{Same-spin $\beta\beta$.}
\begin{equation}
E^{(2)}_{\beta\beta} = \frac{1}{2} \sum_{IJAB} \langle IJ|AB\rangle \, T_{IJ}^{AB,\beta\beta}.
\end{equation}

\paragraph{Opposite-spin $\alpha\beta$.}
No exchange, no antisymmetry, no overcounting:
\begin{equation}
E^{(2)}_{\alpha\beta} = \sum_{iJaB} \frac{\langle iJ|aB\rangle^2}{\Delta_{iJaB}^{\alpha\beta}}
= \sum_{iJaB} \langle iJ|aB\rangle \, T_{iJ}^{aB,\alpha\beta}.
\end{equation}

\subsection{Final UHF--MP2 Formula}
\begin{equation}
\label{eq:uhf-mp2}
\boxed{E^{(2)}_{\mathrm{UHF}} = \frac{1}{2} \sum_{ijab} \langle ij|ab\rangle \, T_{ij}^{ab,\alpha\alpha}
+ \frac{1}{2} \sum_{IJAB} \langle IJ|AB\rangle \, T_{IJ}^{AB,\beta\beta}
+ \sum_{iJaB} \langle iJ|aB\rangle \, T_{iJ}^{aB,\alpha\beta},}
\end{equation}
with amplitudes
\begin{equation}
\begin{aligned}
T_{ij}^{ab,\alpha\alpha} &= \frac{\langle ij|ab\rangle - \langle ij|ba\rangle}
{\varepsilon_i^\alpha + \varepsilon_j^\alpha - \varepsilon_a^\alpha - \varepsilon_b^\alpha}, \\[4pt]
T_{IJ}^{AB,\beta\beta} &= \frac{\langle IJ|AB\rangle - \langle IJ|BA\rangle}
{\varepsilon_I^\beta + \varepsilon_J^\beta - \varepsilon_A^\beta - \varepsilon_B^\beta}, \\[4pt]
T_{iJ}^{aB,\alpha\beta} &= \frac{\langle iJ|aB\rangle}
{\varepsilon_i^\alpha + \varepsilon_J^\beta - \varepsilon_a^\alpha - \varepsilon_B^\beta}.
\end{aligned}
\end{equation}
...
A correction vector $\vect{u}^{\st{n}} \rightarrow \vect{u}^{k+1}$ is computed and added to the subspace. In \elemcoil, 
we use either the diagonal preconditioner,
\beq
