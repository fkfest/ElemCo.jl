...
\cdk{I don't understand this. I guess, it's about the mean-field Fock operator. But what does tilde mean?}
\cks{I changed the text below. I modified Eqs. (\ref{eq:df-correction}) and (\ref{eq:df-correction-fock}) to include the positron contributions. The tilde is used as in those equations}



With positrons, the density-fitted equations (\ref{eq:df-correction}) and (\ref{eq:df-correction-fock}) are modified to include the positron contributions:
\begin{equation}
  \begin{aligned}
    \tilde h_p^q &= \tilde f_p^q 
...
\cdk{In \eqs{eq:poshf-fock-e}{eq:poshf-fock-p} the $D$ already contains the factor of 2. Should it be removed from the equations?}
\cks{Thanks, Daniel, and well spot. I removed it from fock operator equations. I checked the code, and there was no extra factor of 2, and the code was like the equations above. Note that I also changed the HF energy of e-p system below, and now it corresponds to the code and is also correct, I would say.}
