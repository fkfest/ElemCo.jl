...
\begin{aligned}
  ...
  \forall \hat{R} &= \sum_{r\mathchar"313E k}R_r^k(\hat{E}_r^k-\hat{E}_k^r),  \bf{R}=-\bf{R}^{\dagger},\\
\end{aligned}
\end{equation}
In which $\hat{E}_r^k$ is the singlet excitation operator. $R_r^k$ is the element of matrix $\bf{R}$, here we let it be real, thus $\bf{R}$ matrix is an antisymmetric matrix, with $R_r^k=-R_k^r$. Mathemetically it can be proved that the transforming matrix $\bf{U}$ is unitary.
...
\begin{equation}
E=\sum_{tu}{}^{c}F_t^uD_t^u+\frac{1}{2}\sum_{tuvw}v^{uw}_{tv}D_{tv}^{uw}+E_c+E_{nuc},
\end{equation}
with
\begin{equation}
\begin{aligned}
  E_c=&\sum_j[h^j_j+{}^cF_j^j],\\
  {}^cF_r^s =& h^s_r+\sum_j[(2v^{sj}_{rj}-v^{js}_{rj})], 
\end{aligned}
\end{equation}
$E_c$ is the closed shell electronic energy, $\bf{{}^cF}$ is the closed shell part of Fock matrix, and 1-particle density matrix $\bf{{}^1D}$ and symmetrized  2-particle density matrix $\bf{{}^2D}$ are defined as 
\begin{equation}
\begin{aligned}
  {}^1D_t^u =& \sum_{IJ} c_Ic_J<\Phi_I|\hat{E}_t^u|\Phi_J>,\\
  {}^2D_{tv}^{uw} =& \sum_{IJ} c_Ic_J<\Phi_I|\frac{1}{2}[\hat{E}_{tv}^{uw}+\hat{E}_{uv}^{tw}]|\Phi_J>,
\end{aligned}
\end{equation}
with $\hat{E}_{tv}^{uw}$ as the 2-electron excitation operator. In order to simplify the expression, the ${}^1D_v^u$ is written as $D_v^u$, and ${}^2D_{tv}^{uw}$ is written as $D_{tv}^{uw}$ in follow. Here, the 4-index integral $v^{uw}_{tv}$ is calculated as the density fitted integrals, as mentioned in previous chapter,
\begin{equation}
\begin{aligned}
...
\begin{aligned}
  A_r^i=&2F_r^i,\\
  A_r^u=&\sum_v {}^cF_r^v D_v^u + \sum_{tvw}v^{tw}_{rv}D_{tv}^{uw},\\
  A_r^a=&0,\\
\end{aligned}
\end{equation}
with
\begin{equation}
  F_r^s = {}^cF_r^s + \sum_{tu}D_t^u[v^{su}_{rt}-\frac{1}{2}v^{ts}_{ru}].
\end{equation}
Since matrix $\bf{g}$ is asymmetric, $g_{rk}=-g_{kr}$, and we don't need the internal rotation parts of doubly occupied orbitals and virtual orbitals, we exclude the redundant parts of $\bf{g}$ and $\bf{h}$. Thus $\bf{g}$ is a combination of only $[\bf{g}_t^j,\bf{g}_a^j,\bf{g}_t^u, \bf{g}_a^u ]$.
...
\end{equation}
\begin{equation}
{}^{SCI}H_{rs}^{kl}=2<rk|\hat{H}^{eff} - E^{(0)}|sl>.
\end{equation}
More specifically,
\begin{equation}
\begin{aligned}
  {}^{SCI}H_{ab}^{ij} =& 4(\delta_{ij}F_a^b-\delta_{ab}F_i^j),\\
  {}^{SCI}H_{ab}^{iu} =& -2\delta_{ab}F_i^vD_v^u,\\
  {}^{SCI}H_{au}^{ij} =& \delta_{ij}(4F_a^u-2F_a^vD_v^u),\\
  {}^{SCI}H_{tb}^{iu} =& 0,\\
  {}^{SCI}H_{tu}^{ij} =& (2D_t^u-4\delta_{tu}) F_i^j\\
                   &+2\delta_{ij}[2F_t^u-(D_{tv}^{uw}-D_t^uD_v^w)F_v^w\\
                   &-D_t^vF_v^u-F_t^vD_v^u],\\
  {}^{SCI}H_{ab}^{tu} =&2\delta_{ab}(D_{tv}^{uw}-D_t^uD_v^w)F_v^w + 2D_t^uF_a^b.
\end{aligned}
\end{equation}
...
\begin{equation}
  {}^{SO}H_{rs}^{kl} = (1-\tau_{rk})(1-\tau_{sl})[2G^{kl}_{rs}-\delta_{kl}(A_r^s+A_s^r)],
\end{equation}
...
G^{tu}_{rs} =& {}^cF_r^sD_t^u + [v^{sw}_{rv}D_{tv}^{uw}+2v^{vw}_{rs}D_{tu}^{vw}],\\
\end{aligned}
\end{equation}
where
\begin{equation}
