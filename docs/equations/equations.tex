...
\section{Exact AO integrals: the $\pm$ supermatrix store} \label{sec:pm-store}
The exact (non-density-fitted) four-index AO integrals are stored as the $\pm$
supermatrices, the only representation from which both Hartree--Fock
(Sec.~\ref{sec:ao-hf}) and the AO-driven coupled-cluster methods
(Sec.~\ref{sec:ao-direct}) contract directly.

Real AO integrals possess the full eight-fold permutational symmetry, which would in principle
permit an $n^4/8$ packing. The store, however, is built on the smaller symmetry group that
survives for \emph{complex} and for similarity-transformed integrals: the \emph{joint} exchange
symmetry
\begin{equation}
\label{eq:ao-joint-sym}
v_{\mu\nu}^{\rho\sigma} = v_{\nu\mu}^{\sigma\rho}
\end{equation}
together with hermiticity, $v_{\mu\nu}^{\rho\sigma} = \left(v_{\rho\sigma}^{\mu\nu}\right)^*$.
In this group neither the bra pair nor the ket pair may be packed triangularly on its own. Splitting the
integrals into the parts that are symmetric and antisymmetric with respect to the exchange of the
bra pair,
\begin{equation}
\label{eq:pm-def}
V^{\pm}_{(\mu\nu)(\rho\sigma)} = v_{\mu\nu}^{\rho\sigma} \pm v_{\nu\mu}^{\rho\sigma},
\qquad \mu \le \nu,\quad \rho \le \sigma,
\end{equation}
converts the single joint symmetry \eq{eq:ao-joint-sym} into two independent ones: together with the
hermiticity of the integrals, both $V^{+}$ and $V^{-}$ are symmetric (hermitian for complex
integrals) matrices in the packed pair space,
\begin{equation}
V^{\pm}_{(\rho\sigma)(\mu\nu)} = \left(V^{\pm}_{(\mu\nu)(\rho\sigma)}\right)^*,
\end{equation}
so that \emph{both} index pairs are triangular. Only the lower block triangle has to be stored, and
the store occupies $\approx n^4/4$ instead of the $n^4/2$ of the joint triangular packing.

\subsection{Storage layout and generation}
Both $V^{+}$ and $V^{-}$ are kept as their lower \emph{block} triangles over the packed pair
space, written as $\sigma$-aligned column panels; the upper part is the (conjugate-)hermitian
mirror, reconstructed at read time. The $\pm$ fold is fused into the integral generation: each
shell-aligned ket-column block is assembled in a bounded slab of memory, folded according to
\eq{eq:pm-def} and written directly as panels, so no jointly packed $n^4/2$ intermediate exists at
any point, on disk or in memory. Since the stored rows of a panel are bounded below by the panel's
own block, the ERI kernel skips every bra shell pair lying entirely below that row floor
(the kept-pair predicate $\max(\mu,\nu)\ge\sigma_{\mathrm{block\,start}}$ is exact for
shell-aligned blocks), which removes roughly half of the integral evaluations without any
screening approximation.

The half-transformed store of \eq{eq:ao-halftrans} is kept pair-packed as well,
$\left[(i\nu),\, o,\, (\rho\sigma)\right]$ with $\rho\le\sigma$ and both ket orders stored
($o=1$: $(\rho,\sigma)$; $o=2$: $(\sigma,\rho)$) -- the layout in which each output pair block
owns its column range outright, so the out-of-core gather writes every block exactly once. By the
exchange symmetry \eq{eq:ao-joint-sym} a single file serves both half-transform roles of
\eq{eq:ao-occ-early-half}. The gather reads each $\pm$ element approximately twice (the native
panel sequentially through the memory map, the mirror rows of earlier panels by positional reads
with read-ahead advice), and the store is subsequently streamed by whole $\sigma$-columns for the
integral blocks of the correlated methods.
...
\section{Hartree--Fock from exact AO integrals} \label{sec:ao-hf}
When the exact four-index AO integrals are available as the $\pm$ supermatrices of
Sec.~\ref{sec:pm-store}, the Fock matrix is assembled directly from the stored lower block
triangle, without ever forming the full $v_{\mu\nu}^{\rho\sigma}$ tensor. With the (spin-summed)
AO density $\gamma_{\nu}^{\sigma} = 2\,C_{\nu}^{i} C^{\dagger\sigma}_{i}$ the closed-shell Fock
matrix is
\begin{equation}
f_\mu^\nu = h_\mu^\nu + J_\mu^\nu - \half K_\mu^\nu,
\qquad
J_\mu^\nu = v_{\mu\rho}^{\nu\sigma}\, \gamma_{\sigma}^{\rho},
\qquad
K_\mu^\nu = v_{\mu\rho}^{\sigma\nu}\, \gamma_{\sigma}^{\rho}.
\end{equation}
The store is streamed once per SCF iteration, one ket-pair column $(\rho\sigma)$, $\rho\le\sigma$,
at a time: the dense slab $v_{\mu\nu}^{\rho\sigma}$ is reconstructed from
$\half(V^{+}+V^{-})$ and its hermitian mirror, and contributes the band products
\begin{equation}
\label{eq:ao-jk-slab}
J_\mu^{\rho} \mathrel{+}= v_{\mu\nu}^{\rho\sigma}\,\gamma_{\nu}^{\sigma},\qquad
J_\mu^{\sigma} \mathrel{+}= v_{\nu\mu}^{\rho\sigma}\,\gamma_{\nu}^{\rho},\qquad
K_\mu^{\sigma} \mathrel{+}= v_{\mu\nu}^{\rho\sigma}\,\gamma_{\nu}^{\rho},\qquad
K_\mu^{\rho} \mathrel{+}= v_{\nu\mu}^{\rho\sigma}\,\gamma_{\nu}^{\sigma},
\end{equation}
so the working memory is $\mathcal{O}(n^2)$ regardless of the basis size. For a real symmetric
density (the HF case, $\gamma = \gamma^{T}$) the Fock matrices are symmetric and the mirror
contributions of \eq{eq:ao-jk-slab} are the transposes of the native ones; they are skipped
entirely and restored at the end by the symmetrization
\begin{equation}
A \leftarrow A + A^{T} + A_{\mathrm d},
\end{equation}
where $A_{\mathrm d}$ collects the (self-mirroring) diagonal-tile band separately to avoid double
counting -- approximately halving the number of band products. The unrestricted Fock matrices are
built in the \emph{same} single sweep: the shared Coulomb term from
$\pre{^\alpha}\gamma + \pre{^\beta}\gamma$ and the two same-spin exchange terms from
$\pre{^\alpha}\gamma$ and $\pre{^\beta}\gamma$ per slab.
...
\section{AO-driven coupled cluster} \label{sec:ao-direct}
In the \textsf{cckext} factorization of Sec.~\ref{sec:cs-ccsd} the only quantity that requires integrals
with two general indices in the bra is $K^{ij}_{pq}$, \eq{eq:kext-K}; every other integral block
appearing in the amplitude, $\Lambda$, perturbative-triples and EOM equations carries at least one
occupied index. This makes it possible to evaluate the equations \emph{directly} from the AO
integrals $v_{\mu\nu}^{\rho\sigma}$, without ever constructing the MO integrals, and thus without
the $\mathcal{O}(N^5)$ four-index transformation and without storing an $\mathcal{O}(N^4)$ MO
integral set. Below $\mu,\nu,\rho,\sigma$ denote AO indices, $n$ the number of AOs and $o$ the
number of correlated occupied orbitals.

\subsection{Singles dressing as a coefficient transformation}
Only lower virtual and upper occupied indices are dressed (Sec.~\ref{sec:cs-ccsd}), and the dressing is
linear in each index separately. It can therefore be absorbed entirely into two sets of
coefficients, one for the lower (bra) and one for the upper (ket) indices,
\begin{equation}
\label{eq:ao-dressed-coeffs}
\begin{aligned}
L^\mu_p &= \begin{cases} C^\mu_i, & p = i\\[2pt] C^\mu_a - C^\mu_k T^k_a, & p = a\end{cases}
&\qquad
R_\mu^p &= \begin{cases} C_\mu^i + C_\mu^c T^i_c, & p = i\\[2pt] C_\mu^a, & p = a,\end{cases}
\end{aligned}
\end{equation}
so that every dressed integral is an \emph{undressed} AO integral transformed with $L$ on the lower
and $R$ on the upper indices,
\begin{equation}
\label{eq:ao-dressed-ints}
\hat v_{pq}^{rs} = L^\mu_p L^\nu_q\, v_{\mu\nu}^{\rho\sigma}\, R_\rho^r R_\sigma^s.
\end{equation}
The two sets are biorthogonal with respect to the AO overlap $S_\mu^\nu$,
\begin{equation}
\label{eq:ao-biorthogonal}
L^\mu_p\, S_\mu^\nu\, R_\nu^q = δ_p^q,
\end{equation}
which follows from the orthonormality of the MOs, $C^\mu_p S_\mu^\nu C_\nu^q = δ_p^q$, together
with the fact that the occupied--virtual blocks of \eq{eq:ao-dressed-coeffs} cancel:
$-T^k_a δ_k^j$ from $L$ against $+δ_a^c T^j_c$ from $R$. \eq{eq:ao-biorthogonal} is the statement
that the dressing is a similarity transformation. Consequently the singles never enter as a
separate dressing step: each iteration simply rebuilds \eq{eq:ao-dressed-coeffs} and re-contracts
the AO integrals.

\subsection{Half-transformed integrals}
The lower occupied coefficients in \eq{eq:ao-dressed-coeffs} are \emph{undressed}, $L^\mu_i =
C^\mu_i$. The half-transformed integrals
\begin{equation}
\label{eq:ao-halftrans}
v_{i\nu}^{\rho\sigma} = C^\mu_i\, v_{\mu\nu}^{\rho\sigma}
\end{equation}
are therefore independent of $T_1$ and are constructed \emph{once} per set of orbitals -- by the
out-of-core gather from the $\pm$ store described in Sec.~\ref{sec:pm-store}, which keeps them
pair-packed in both ket orders. Every block with at least one occupied index is then obtained from
\eq{eq:ao-halftrans} by matrix multiplications alone, at a cost of $\mathcal{O}(o\,n^3)$ per block;
the per-iteration data volume drops accordingly, from the $\approx n^4/4$ elements of the $\pm$
triangle to the $\approx o\,n^3$ elements of the half-transformed store, and the $\pm$ store itself
is streamed per iteration only by the four-external term
(Sec.~\ref{sec:pm-supermatrix-use}). Blocks whose only occupied index sits on an upper (ket) slot are reached through
\begin{equation}
v_{pq}^{rs} = \left(v_{rs}^{pq}\right)^*,
\end{equation}
which for real integrals is the plain bra--ket exchange.


\subsection{One-pass occupied-early dressing}
The T$_1$-dressed integral blocks that the CCSD/DCSD residual reads (all carrying at least one
occupied index) are assembled once per iteration in a \emph{single} pass over the half-transformed
store: for every ket column $\sigma$ the slabs
\begin{equation}
\label{eq:ao-occ-early-half}
h^{(\rho\sigma)}_{i\nu} = C^{\mu}_{i}\, v_{\mu\nu}^{\rho\sigma},
\qquad
\bar h^{(\rho\sigma)}_{i\mu} = C^{\nu}_{i}\, v_{\mu\nu}^{\rho\sigma}
\end{equation}
are \emph{read} -- both roles from the same file, by the exchange symmetry \eq{eq:ao-joint-sym} --
and the closed-shell sweep accumulates the three occupied-early intermediates
\begin{equation}
\label{eq:ao-occ-early}
v_{ij}^{\rho\sigma} = h^{(\rho\sigma)}_{i\nu} C^{\nu}_{j},
\qquad
v_{i\nu}^{j\sigma} = h^{(\rho\sigma)}_{i\nu}\, R_{\rho}^{j},
\qquad
v_{\mu i}^{j\sigma} = \bar h^{(\rho\sigma)}_{i\mu}\, R_{\rho}^{j},
\end{equation}
with the dressed occupied-ket coefficients $R^{j}_\rho$ of \eq{eq:ao-dressed-coeffs}; the
open-shell sweep carries two coefficient sets and five such intermediates. Every dressed block of
the residual (and the dressed Fock matrix) then follows from \eq{eq:ao-occ-early} by contracting
the remaining AO indices with $L$ and $R$, and the ket contractions over $\rho$ are batched into
level-3 BLAS products over accumulated slabs rather than performed as per-slab rank-one updates.
Each element of the half-transformed store is read exactly once per iteration.

When the half-transformed store has not been built (a single-shot doubles-only calculation, or its
per-macro-iteration rebuild in orbital-optimized methods), the same intermediates are formed in one
pass over the $\pm$ triangle instead, with the band half-transforms \eq{eq:ao-occ-early-half}
computed on the fly from each reconstructed slab -- the identical sweep at $\approx n^4/4$
streamed elements.

\subsection{Four-external term from the $\pm$ supermatrices} \label{sec:pm-supermatrix-use}
With the $\pm$ supermatrices of Sec.~\ref{sec:pm-store} both index pairs of the AO
integrals are packed triangularly and only the lower block triangle is stored.

With the effective pair density of \eq{eq:kext-K} transformed to the AO basis, $D^{ij}_{\rho\sigma}$,
and folded in the same way,
\begin{equation}
\label{eq:pm-density}
D^{\pm,ij}_{(\rho\sigma)} = \half\left(D^{ij}_{\rho\sigma} \pm D^{ij}_{\sigma\rho}\right)\quad(\rho\lt\sigma),
\qquad D^{+,ij}_{(\rho\rho)} = \half D^{ij}_{\rho\rho},\qquad D^{-,ij}_{(\rho\rho)} = 0,
\end{equation}
the four-external term reduces to two matrix products in the packed pair space,
\begin{equation}
\label{eq:pm-kext}
\begin{aligned}
K^{ij}_{\mu\nu} &= \left(V^{+}D^{+,ij} + V^{-}D^{-,ij}\right)_{(\mu\nu)},\\
K^{ij}_{\nu\mu} &= \left(V^{+}D^{+,ij} - V^{-}D^{-,ij}\right)_{(\mu\nu)},
\end{aligned}
\end{equation}
i.e.\ \emph{both} orders of the bra pair are obtained from the same pair of products. The MO-basis
$K^{ij}_{pq}$ of \eq{eq:kext-K} follows by contracting the remaining pair with $L$.

\subsection{Occupied-pair packing and operation count}
The effective pair density satisfies $D^{ij}_{\rho\sigma} = D^{ji}_{\sigma\rho}$, which together
with \eq{eq:ao-joint-sym} gives
\begin{equation}
\label{eq:kext-ij-sym}
K^{ij}_{pq} = K^{ji}_{qp}.
\end{equation}
Because \eq{eq:pm-kext} delivers both bra orders simultaneously, \eq{eq:kext-ij-sym} allows the
occupied pair to be restricted to $i \le j$ as well. With $n_{pp} = n(n+1)/2$ and
$n_{oo} = o(o+1)/2$ the $\pm$ form therefore requires
\begin{equation}
2\, n_{pp}^2\, n_{oo}
\end{equation}
multiplications, whereas the contraction over the jointly packed integrals requires
$n^2 n_{pp} o^2$: there the ket pair alone is packed, the result is recovered as
$K^{ij}_{pq} = A^{ij}_{pq} + A^{ji}_{qp}$ from a partial contraction $A$, and this symmetrization
consumes \emph{both} occupied orders, so the occupied triangle cannot be exploited. The ratio is
\begin{equation}
\label{eq:pm-flop-ratio}
\frac{n^2 n_{pp} o^2}{2\,n_{pp}^2\,n_{oo}} = \frac{2no}{(n+1)(o+1)}
\;\xrightarrow[\;n,o\gg 1\;]{}\; 2,
\end{equation}
the product of the bra-pair packing ($n^2 \to n_{pp}$), the two products instead of one, and the
occupied-pair packing ($o^2 \to n_{oo}$). Both the operation count and the amount of data streamed
are thus halved. Note that the second factor in \eq{eq:pm-flop-ratio} approaches unity only for a
sufficiently large occupied space: $n_{oo}$ is the width of the right-hand side of the matrix
products \eq{eq:pm-kext}, and for small $o$ the products are too narrow to reach the asymptotic
performance of the underlying matrix multiplication.

The same supermatrices are used for the closed- and open-shell Fock-like intermediates, which are
built by streaming $V^{\pm}$ once, and — through \eq{eq:ao-halftrans} — for the $\Lambda$,
perturbative-triples and EOM integral blocks, so that the AO integrals are the only integral set
that is ever stored.
