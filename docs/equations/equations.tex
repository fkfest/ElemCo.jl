...
E^{(2)} = \frac{1}{4} \sum_{ijab} \frac{(v_{ij}^{ab} - v_{ij}^{ba})^2}
...
= 2 \times \frac{1}{4} \sum_{ijab} \frac{(v_{ij}^{ab} - v_{ij}^{ba})^2}{\Delta_{ijab}}.
\end{equation}
...
= 2 \times \frac{1}{2} \sum_{ijab} \frac{(v_{ij}^{ab})^2}{\Delta_{ijab}}.
\end{equation}
...
\begin{equation}
(v_{ij}^{ab} - v_{ij}^{ba})^2 
= (v_{ij}^{ab})^2 - 2v_{ij}^{ab}v_{ij}^{ba} + (v_{ij}^{ba})^2.
\end{equation}
Since $\sum_{ijab} (v_{ij}^{ba})^2 = \sum_{ijab} (v_{ij}^{ab})^2$ 
by relabeling $a \leftrightarrow b$:
\begin{equation}
\sum_{ijab} (v_{ij}^{ab} - v_{ij}^{ba})^2 
= 2 \sum_{ijab} \bigl((v_{ij}^{ab})^2 - v_{ij}^{ab}v_{ij}^{ba}\bigr).
\end{equation}
...
E^{(2)}_{\mathrm{SS}} = \frac{1}{2} \cdot 2 \sum_{ijab} 
  \frac{(v_{ij}^{ab})^2 - v_{ij}^{ab}v_{ij}^{ba}}{\Delta_{ijab}}
= \sum_{ijab} \frac{v_{ij}^{ab}(v_{ij}^{ab} - v_{ij}^{ba})}{\Delta_{ijab}}.
\end{equation}
The opposite-spin contribution simplifies to:
\begin{equation}
E^{(2)}_{\mathrm{OS}} = \sum_{ijab} \frac{(v_{ij}^{ab})^2}{\Delta_{ijab}}.
\end{equation}
...
&= \sum_{ijab} \frac{(v_{ij}^{ab})^2 - v_{ij}^{ab}v_{ij}^{ba} 
   + (v_{ij}^{ab})^2}{\Delta_{ijab}} \\[4pt]
&= \sum_{ijab} \frac{2(v_{ij}^{ab})^2 - v_{ij}^{ab}v_{ij}^{ba}}{\Delta_{ijab}}.
\end{aligned}
\end{equation}
Factoring:
\begin{equation}
...
\frac{v_{ij}^{ab} (2v_{ij}^{ab} - v_{ij}^{ba})}{\Delta_{ijab}}.}
\end{equation}

\subsection{Amplitude Form}
Using spatial-orbital amplitudes $T_{ij}^{ab} = v_{ij}^{ab} / \Delta_{ijab}$:
\begin{equation}
\begin{aligned}
E^{(2)}_{\mathrm{SS}} &= \sum_{ijab} v_{ij}^{ab} (T_{ij}^{ab} - T_{ij}^{ba}), \\
E^{(2)}_{\mathrm{OS}} &= \sum_{ijab} v_{ij}^{ab} \, T_{ij}^{ab}, \\
E^{(2)}_{\mathrm{RHF}} &= \sum_{ijab} v_{ij}^{ab} (2T_{ij}^{ab} - T_{ij}^{ba}).
\end{aligned}
\end{equation}
...
\frac{(v_{ij}^{ab} - v_{ij}^{ba})^2}{\Delta_{ijab}^\alpha}
= \frac{1}{2} \sum_{ijab} v_{ij}^{ab} \, T_{ij}^{ab,\alpha\alpha}.
\end{equation}
The $\tfrac{1}{4} \to \tfrac{1}{2}$ follows from amplitude antisymmetry.

\paragraph{Same-spin $\beta\beta$.}
\begin{equation}
E^{(2)}_{\beta\beta} = \frac{1}{2} \sum_{\spb{i}\spb{j}\spb{a}\spb{b}} v_{\spb{i}\spb{j}}^{\spb{a}\spb{b}} \, T_{\spb{i}\spb{j}}^{\spb{a}\spb{b},\beta\beta}.
\end{equation}

\paragraph{Opposite-spin $\alpha\beta$.}
No exchange, no antisymmetry, no overcounting:
\begin{equation}
E^{(2)}_{\alpha\beta} = \sum_{i\spb{j}a\spb{b}} \frac{(v_{i\spb{j}}^{a\spb{b}})^2}{\Delta_{i\spb{j}a\spb{b}}^{\alpha\beta}}
= \sum_{i\spb{j}a\spb{b}} v_{i\spb{j}}^{a\spb{b}} \, T_{i\spb{j}}^{a\spb{b},\alpha\beta}.
\end{equation}

\subsection{Final UHF--MP2 Formula}
\begin{equation}
\label{eq:uhf-mp2}
\boxed{E^{(2)}_{\mathrm{UHF}} = \frac{1}{2} \sum_{ijab} v_{ij}^{ab} \, T_{ij}^{ab,\alpha\alpha}
+ \frac{1}{2} \sum_{\spb{i}\spb{j}\spb{a}\spb{b}} v_{\spb{i}\spb{j}}^{\spb{a}\spb{b}} \, T_{\spb{i}\spb{j}}^{\spb{a}\spb{b},\beta\beta}
+ \sum_{i\spb{j}a\spb{b}} v_{i\spb{j}}^{a\spb{b}} \, T_{i\spb{j}}^{a\spb{b},\alpha\beta},}
\end{equation}
with amplitudes
\begin{equation}
\begin{aligned}
T_{ij}^{ab,\alpha\alpha} &= \frac{v_{ij}^{ab} - v_{ij}^{ba}}
{\varepsilon_i^\alpha + \varepsilon_j^\alpha - \varepsilon_a^\alpha - \varepsilon_b^\alpha}, \\[4pt]
T_{\spb{i}\spb{j}}^{\spb{a}\spb{b},\beta\beta} &= \frac{v_{\spb{i}\spb{j}}^{\spb{a}\spb{b}} - v_{\spb{i}\spb{j}}^{\spb{b}\spb{a}}}
{\varepsilon_{\spb{i}}^\beta + \varepsilon_{\spb{j}}^\beta - \varepsilon_{\spb{a}}^\beta - \varepsilon_{\spb{b}}^\beta}, \\[4pt]
T_{i\spb{j}}^{a\spb{b},\alpha\beta} &= \frac{v_{i\spb{j}}^{a\spb{b}}}
{\varepsilon_i^\alpha + \varepsilon_{\spb{j}}^\beta - \varepsilon_a^\alpha - \varepsilon_{\spb{b}}^\beta}.
\end{aligned}
\end{equation}
...
Electron indices remain undecorated as defined in the Notation section.
...
\begin{equation}
  \begin{aligned}
    ...
    - v_{p}^{\tilde i\tilde L} v_{\tilde i}^{q\tilde L} - v_{p}^{q\tilde L} v_{\pos{i}}^{\pos{i}\tilde L} \right),\\
  \end{aligned}
\end{equation}
where $\pos{i}=1$ denote the single occupied positron orbital.The positron one-body terms get corrections according to 
\begin{equation}
  \begin{aligned}
    \tilde h_{\pos{p}}^{\pos{q} (\mathbf{+})} &= \tilde f_{\pos{p}}^{\pos{q} (\mathbf{+})} 
    + \sum_L \left(2v_{i}^{iL}v_{\pos{p}}^{\pos{q}L}\right)\\
    \tilde f_{\pos{p} }^{\pos{q} (\mathbf{+})} &= h_{\pos{p}}^{\pos{q} (\mathbf{+})} - \sum_L \left(2v_{ \tilde i}^{ \tilde i \tilde L}v_{\pos{p}}^{\pos{q} \tilde L}\right), 
    \\
  \end{aligned}
\end{equation}
...
The electron--positron integral $v_{i\pos{i}}^{a\pos{a}}$ describes 
electron~1 in orbital~$i$ and positron~2 in orbital~$\pos{i}$ scattering to 
virtual orbitals $a$ and $\pos{a}$. The attractive electron--positron interaction 
has the opposite sign to electron--electron repulsion; in the code, the \textit{epep}-elements are saved with same sign as \textit{eeee}-elements, and the sign is taken into account in the algebra.
...
\begin{equation}
E^{(2)}_{\mathrm{ep}} = \sum_{i\pos{i}a\pos{a}} 
\frac{(v_{i_\sigma \pos{i}_\tau}^{a_\sigma \pos{a}_\tau})^2}
{\varepsilon_i + \varepsilon_{\pos{i}} - \varepsilon_a - \varepsilon_{\pos{a}}},
\end{equation}
where spin is conserved independently for each particle type 
($\sigma_i = \sigma_a$ and $\tau_{\pos{i}} = \tau_{\pos{a}}$).

\subsection{Spin Integration}
For RHF electrons, the $\alpha$ and $\beta$ spatial orbitals are identical.
The single positron occupies one spatial orbital $\pos{i}$ with a definite spin 
(say $\alpha$). There are two contributing spin configurations:
\begin{itemize}
\item $\alpha$-electron with $\alpha$-positron: $i \pos{i}\to a \pos{a}$
\item $\beta$-electron with $\alpha$-positron: $\spb{i} \pos{i} \to \spb{a} \pos{a}$
\end{itemize}
Both give the same spatial integral $v_{i\pos{i}}^{a\pos{a}}$. Summing over 
electron spin:
\begin{equation}
E^{(2)}_{\mathrm{ep}} = 2 \sum_{i\pos{i}a\pos{a}} 
\frac{(v_{i\pos{i}}^{a\pos{a}})^2}{\Delta_{i\pos{i}a\pos{a}}},
\end{equation}
...
\begin{equation}
T_{i\pos{i}}^{a\pos{a}} = \frac{v_{i\pos{i}}^{a\pos{a}}}{\Delta_{i\pos{i}a\pos{a}}},
\end{equation}
the electron--positron correlation energy becomes:
\begin{equation}
\label{eq:ep-mp2}
\boxed{E^{(2)}_{\mathrm{ep}} = 2 \sum_{i\pos{i}a\pos{a}} v_{i\pos{i}}^{a\pos{a}} \, T_{i\pos{i}}^{a\pos{a}}.}
\end{equation}
