...
\end{equation}
are therefore independent of $T_1$ and can be constructed once per set of orbitals rather than once
per iteration. Every block with at least one occupied index is then obtained from
\eq{eq:ao-halftrans} by matrix multiplications alone, at a cost of $\mathcal{O}(o\,n^3)$ per block
instead of a fresh pass over the AO integrals. Blocks whose only occupied index sits on an upper (ket) slot are reached through
\begin{equation}
v_{pq}^{rs} = \left(v_{rs}^{pq}\right)^*,
\end{equation}
which for real integrals is the plain bra--ket exchange.

\subsection{Four-external term: $\pm$ supermatrices} \label{sec:pm-supermatrix}
The AO integrals possess only the \emph{joint} exchange symmetry
\begin{equation}
\label{eq:ao-joint-sym}
v_{\mu\nu}^{\rho\sigma} = v_{\nu\mu}^{\sigma\rho},
\end{equation}
i.e.\ neither the bra pair nor the ket pair may be packed triangularly on its own. Splitting the
