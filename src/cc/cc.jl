...
# Function to calculate length for buffer buf1
# autogenerated by @print_buffer_usage
function auto_buf1_length4calc_dressed_ints(no1, no2, nv1, nv2, calc_d_vvvv, calc_d_vvvo, calc_d_vovv, calc_d_vvoo, mixed)
    lenbuf1 = Ref(0)
    peakbuf1 = Ref(0)
    if calc_d_vvvv
        hd_vvvv = pseudo_alloc!(lenbuf1, peakbuf1, nv1 * nv2 * nv1 * nv2)
        pseudo_drop!(lenbuf1, hd_vvvv)
    end
    hd_oooo = pseudo_alloc!(lenbuf1, peakbuf1, no1 * no2 * no1 * no2)
    if mixed
        hd_oooo2 = pseudo_alloc!(lenbuf1, peakbuf1, no1 * no2 * no1 * no2)
    end
    if calc_d_vvoo
        hd_vvoo = pseudo_alloc!(lenbuf1, peakbuf1, nv1 * nv2 * no1 * no2)
        voov = pseudo_alloc!(lenbuf1, peakbuf1, nv1 * no2 * no1 * nv2)
        pseudo_drop!(lenbuf1, voov)
        pseudo_drop!(lenbuf1, hd_vvoo)
    end
    if !mixed
        nothing
    else
        pseudo_drop!(lenbuf1, hd_oooo2)
    end
    if no2 > 0
        vovo = pseudo_alloc!(lenbuf1, peakbuf1, nv1 * no2 * nv1 * no2)
        pseudo_drop!(lenbuf1, vovo)
    end
    if mixed
        ovov = pseudo_alloc!(lenbuf1, peakbuf1, no1 * nv2 * no1 * nv2)
        pseudo_drop!(lenbuf1, ovov)
    end
    oovv = pseudo_alloc!(lenbuf1, peakbuf1, no1 * no2 * nv1 * nv2)
    pseudo_drop!(lenbuf1, oovv)
    pseudo_drop!(lenbuf1, hd_oooo)
    d_voov = pseudo_alloc!(lenbuf1, peakbuf1, nv1 * no2 * no1 * nv2)
    if mixed
        oOvV = pseudo_alloc!(lenbuf1, peakbuf1, no1 * no2 * nv1 * nv2)
        pseudo_drop!(lenbuf1, oOvV)
    end
    if mixed
        d_ovvo = pseudo_alloc!(lenbuf1, peakbuf1, no1 * nv2 * nv1 * no2)
    end
    hd_vovo = pseudo_alloc!(lenbuf1, peakbuf1, nv1 * no2 * nv1 * no2)
    if mixed
        hd_ovov = pseudo_alloc!(lenbuf1, peakbuf1, no1 * nv2 * no1 * nv2)
    end
    if calc_d_vvvo
        vvvv = pseudo_alloc!(lenbuf1, peakbuf1, nv1 * nv2 * nv1 * nv2)
        pseudo_drop!(lenbuf1, vvvv)
    end
    if mixed
        pseudo_drop!(lenbuf1, hd_ovov)
    end
    pseudo_drop!(lenbuf1, hd_vovo)
    if calc_d_vovv
        oovv = pseudo_alloc!(lenbuf1, peakbuf1, no1 * no2 * nv1 * nv2)
        pseudo_drop!(lenbuf1, oovv)
    end
    if calc_d_vvvv
        d_vvvv = pseudo_alloc!(lenbuf1, peakbuf1, nv1 * nv2 * nv1 * nv2)
        pseudo_drop!(lenbuf1, d_vvvv)
    end
    if mixed
        pseudo_drop!(lenbuf1, d_ovvo)
    end
    pseudo_drop!(lenbuf1, d_voov)
    if calc_d_vvoo
        d_vvoo = pseudo_alloc!(lenbuf1, peakbuf1, nv1 * nv2 * no1 * no2)
        pseudo_drop!(lenbuf1, d_vvoo)
    end
    return peakbuf1[]
end

# Function to calculate length for buffer buf2
# autogenerated by @print_buffer_usage
function auto_buf2_length4calc_dressed_ints(no1, no2, nv1, nv2, calc_d_vvvv, calc_d_vvvo, calc_d_vovv, calc_d_vvoo, mixed)
    lenbuf2 = Ref(0)
    peakbuf2 = Ref(0)
    if calc_d_vvvv
        vovv = pseudo_alloc!(lenbuf2, peakbuf2, nv1 * no2 * nv1 * nv2)
        pseudo_drop!(lenbuf2, vovv)
    end
    if mixed
        oovo = pseudo_alloc!(lenbuf2, peakbuf2, no1 * no2 * nv1 * no2)
        pseudo_drop!(lenbuf2, oovo)
    end
    ooov = pseudo_alloc!(lenbuf2, peakbuf2, no1 * no2 * no1 * nv2)
    pseudo_drop!(lenbuf2, ooov)
    if calc_d_vvoo
        vooo = pseudo_alloc!(lenbuf2, peakbuf2, nv1 * no2 * no1 * no2)
        pseudo_drop!(lenbuf2, vooo)
        vvov = pseudo_alloc!(lenbuf2, peakbuf2, nv1 * nv2 * no1 * nv2)
        pseudo_drop!(lenbuf2, vvov)
    end
    hd_vooo = pseudo_alloc!(lenbuf2, peakbuf2, nv1 * no2 * no1 * no2)
    if mixed
        hd_ovoo = pseudo_alloc!(lenbuf2, peakbuf2, no1 * nv2 * no1 * no2)
    end
    d_oovo = pseudo_alloc!(lenbuf2, peakbuf2, no1 * no2 * nv1 * no2)
    if mixed
        d_ooov = pseudo_alloc!(lenbuf2, peakbuf2, no1 * no2 * no1 * nv2)
    end
    vovv = pseudo_alloc!(lenbuf2, peakbuf2, nv1 * no2 * nv1 * nv2)
    pseudo_drop!(lenbuf2, vovv)
    if mixed
        ovvv = pseudo_alloc!(lenbuf2, peakbuf2, no1 * nv2 * nv1 * nv2)
        pseudo_drop!(lenbuf2, ovvv)
    end
    if calc_d_vvvo
        hd_vvvo = pseudo_alloc!(lenbuf2, peakbuf2, nv1 * nv2 * nv1 * no2)
        pseudo_drop!(lenbuf2, hd_vvvo)
        if mixed
            hd_vvov = pseudo_alloc!(lenbuf2, peakbuf2, nv1 * nv2 * no1 * nv2)
            pseudo_drop!(lenbuf2, hd_vvov)
        end
    end
    if mixed
        pseudo_drop!(lenbuf2, d_ooov)
    end
    pseudo_drop!(lenbuf2, d_oovo)
    if calc_d_vovv
        d_vovv = pseudo_alloc!(lenbuf2, peakbuf2, nv1 * no2 * nv1 * nv2)
        if mixed
            pseudo_drop!(lenbuf2, d_vovv)
            d_ovvv = pseudo_alloc!(lenbuf2, peakbuf2, no1 * nv2 * nv1 * nv2)
        end
    end
    if calc_d_vvvv
        if !mixed
    ...
    if calc_d_vvvo
        d_vvvo = pseudo_alloc!(lenbuf2, peakbuf2, nv1 * nv2 * nv1 * no2)
        pseudo_drop!(lenbuf2, d_vvvo)
        if mixed
            d_vvov = pseudo_alloc!(lenbuf2, peakbuf2, nv1 * nv2 * no1 * nv2)
            pseudo_drop!(lenbuf2, d_vvov)
        end
    end
    if calc_d_vvoo
        hd_vvvo = pseudo_alloc!(lenbuf2, peakbuf2, nv1 * nv2 * nv1 * no2)
    ...
    if mixed
        pseudo_drop!(lenbuf2, hd_ovoo)
    end
    pseudo_drop!(lenbuf2, hd_vooo)
    return peakbuf2[]
end
...
function calc_dressed_ints(EC::ECInfo, T1, T12, o1::Char, v1::Char, o2::Char, v2::Char;
  ...
  lenbuf1 = auto_buf1_length4calc_dressed_ints(no1, no2, nv1, nv2, calc_d_vvvv, calc_d_vvvo, calc_d_vovv, calc_d_vvoo, mixed)
  # println("Allocate buffer buf1: ", lenbuf1)
  buf1 = Buffer(lenbuf1)
  lenbuf2 = auto_buf2_length4calc_dressed_ints(no1, no2, nv1, nv2, calc_d_vvvv, calc_d_vvvo, calc_d_vovv, calc_d_vvoo, mixed)
  # println("Allocate buffer buf2: ", lenbuf2)
  buf2 = Buffer(lenbuf2)
  # @print_buffer_usage buf1 begin
