...
function calc_ccsd_vector_times_Jacobian(EC::ECInfo, U1, U2; dc=false)
  ...
  @mtensor R2[e,f,m,n] += int2[m,n,i,j] * U2[e,f,i,j]
  ...
  @mtensor tT2[e,f,m,n] := 2.0 * T2[e,f,m,n] - T2[e,f,n,m]
  # ``x_m^i = \tilde T^{il}_{cd} v_{ml}^{cd}``
  @mtensor xoo[m,i] := tT2[c,d,i,l] * oovv[m,l,c,d]
  t1 = print_time(EC, t1, "x_m^i = \\tilde T^{il}_{cd} v_{ml}^{cd}",2)
  # ``x_a^e = \tilde T^{kl}_{ac} v_{kl}^{ec}``
  @mtensor xvv[a,e] := tT2[a,c,k,l] * oovv[k,l,e,c]
  ...
  if length(U1) > 0
    ...
    @mtensor R1[e,m] -= 2.0 * (U2[e,b,i,j] * vT_ovoo[m,b,i,j])
    vT_ovoo = nothing
  ...
  if length(U1) > 0
    ...
    @mtensor R1[e,m] -= 2.0 * (U2[e,b,i,j] * int2[b,m,j,i])
...
function calc_dU2(EC::ECInfo, T1, T12, U2, o1='o', v1='v', o2='o', v2='v')
  ...
  if length(T1) > 0
    ...
    @mtensor dU2[SP[v1],SP[o2],:,:][a,j,m,n] = - U2[a,b,m,n] * T12[b,j]
    @mtensor dU2[SP[o1],SP[o2],:,:][i,j,m,n] = dUovoo[i,b,m,n] * T12[b,j]
...
function calc_ccsd_vector_times_Jacobian4spin(EC::ECInfo, U1, U2, U2ab, 
  ...
  @mtensor D2[i,j,k,l] := 0.5 * (U2[c,d,i,j] * T2[c,d,k,l])
  ...
  if length(U1) > 0
    ...
    @mtensor R1[e,m] += D2[m,j,k,l] * d_oovo[k,l,e,j]
    t1 = print_time(EC, t1, "R_e^m += D_{mj}^{kl} \\hat v_{kl}^{ej}",2)
  end
  # ``D_{ib}^{aj} = Λ_{ik}^{ac} T^{jk}_{bc} + Λ_{iK}^{aC} T^{jK}_{bC}``
  T2ab = load4idx(EC, "T_vVoO")
  @mtensor D2[a,b,i,j] := U2[a,c,i,k] * T2[b,c,j,k]
  if isα
    @mtensor D2[a,b,i,j] += U2ab[a,C,i,K] * T2ab[b,C,j,K]
  else
    @mtensor D2[A,B,I,J] += U2ab[c,A,k,I] * T2ab[c,B,k,J]
  end
  T2ab = nothing
  t1 = print_time(EC, t1, "D_{ib}^{aj} = U_{ik}^{ac} T^{jk}_{bc} + U_{iK}^{aC} T^{jK}_{bC}",2)
  if length(R1) > 0
    # ``R_e^m += D_{id}^{el} (\hat v_{ml}^{di}-\hat v_{lm}^{di})``
    @mtensor R1[e,m] += D2[e,d,i,l] * (d_oovo[m,l,d,i] - d_oovo[l,m,d,i])
  ...
  if length(R1) > 0
    ...
    @mtensor R1[e,m] += D2[a,d,m,l] * (int2[a,l,e,d] - int2[a,l,d,e])
  ...
  @mtensor aR2[e,f,m,n] += U2[a,f,i,n] * vT_voov[a,m,i,e]
  vT_voov = nothing
  ...
  if isα
    @mtensor aR2[e,f,m,n] += U2ab[e,B,m,J] * vT_VoOv[B,n,J,f]
  else
    @mtensor aR2[E,F,M,N] += U2ab[b,E,j,M] * vT_VoOv[b,N,j,F]
  end
  vT_VoOv = nothing
  t1 = print_time(EC, t1, "aR^{ef}_{mn} += U_{mJ}^{eB} \\bar y_{Bn}^{Jf}",2)
  # ``R^{ef}_{mn} += aR^{ef}_{mn} + aR^{fe}_{nm} - aR^{ef}_{nm} - aR^{fe}_{mn}``
  @mtensor R2[e,f,m,n] += aR2[e,f,m,n] + aR2[f,e,n,m] - aR2[e,f,n,m] - aR2[f,e,m,n]
  aR2 = nothing
  ...
  if length(R1) > 0
    ...
    @mtensor R1[e,m] -= U2[b,e,j,i] * int2[b,m,j,i]
    int2 = nothing
    ...
    @mtensor R1[e,m] -= 0.5 * (U2[e,b,i,j] * vT_ovoo[m,b,i,j])
    ...
    if isα
      @mtensor R1[e,m] -= U2ab[e,B,i,J] * vT_ovoo[m,B,i,J]
    else
      @mtensor R1[E,M] -= U2ab[b,E,j,I] * vT_ovoo[M,b,I,j]
    end
    vT_ovoo = nothing
...
function calc_ccsd_vector_times_Jacobian4ab(EC::ECInfo, U1a::Matrix{Float64}, U1b::Matrix{Float64}, 
  ...
  if length(U1a) > 0
    ...
    @mtensor R1a[e,m] := - U2ab[e,B,i,J] * int2[m,B,i,J]
    int2 = NOTHING4idx
  else
    R1a = U1a
  end
  if length(U1b) > 0
    ...
    int2 = load4idx(EC, "d_vOoO")
    @mtensor R1b[E,M] := - U2ab[b,E,j,I] * int2[b,M,j,I]
    int2 = NOTHING4idx
  else
    R1b = U1b
  end
  ...
  if !dc
    T2ab = load4idx(EC, "T_vVoO")
    @mtensor int2[a,N,e,J] -= oOvV[k,N,e,D] * T2ab[a,D,k,J]
  end
  @mtensor R2[e,F,m,N] -= U2ab[a,F,m,J] * int2[a,N,e,J]
  ...
  if !dc
    @mtensor int2[m,B,i,F] -= oOvV[m,L,c,F] * T2ab[c,B,i,L]
    T2ab = NOTHING4idx
  end
  @mtensor R2[e,F,m,N] -= U2ab[e,B,i,N] * int2[m,B,i,F]
...
function calc_1RDM(EC::ECInfo, U1, U1os, U2, U2ab, T1, T2, T2ab, spin)
  ...
  if spin == :α
    ...
    if length(U1) > 0
      @mtensor Dvo[a,i] += U1os[C,K] * T2ab[a,C,i,K] 
    end
  else
    @mtensor begin 
    ...
    if length(U1) > 0
      @mtensor Dvo[A,I] += U1os[c,k] * T2ab[c,A,k,I] 
    end
  end
  if length(T1) > 0
    # 1RDM (including all T1 terms)
...
function calc_3ext_times_T2(EC::ECInfo, T2::AbstractArray, o1::Char='o', v1::Char='v', o2::Char='o', v2::Char='v')
  ...
  @mtensor vT_ovoo[m,b,i,j] := int2[b,m,d,c] * T2[d,c,j,i]
...
function calc_3ext_times_T2(EC::ECInfo, T2a::AbstractArray, T2b::AbstractArray, T2ab::AbstractArray)
  ...
  @mtensor vT_oVoO[m,B,i,J] := int2[m,B,c,D] * T2ab[c,D,i,J]
...
function calc_focklike_vT2(EC::ECInfo, T2a::AbstractArray, T2b::AbstractArray, T2ab::AbstractArray)
  ...
  @mtensor vT_VV[A,C] += int2[l,K,d,C] * T2ab[d,A,l,K]
  save!(EC, "vT_oo", vT_oo)
  save!(EC, "vT_vv", vT_vv)
  save!(EC, "vT_OO", vT_OO)
  save!(EC, "vT_VV", vT_VV)
...
function calc_rings_vT2(EC::ECInfo, T2a::AbstractArray, T2b::AbstractArray, T2ab::AbstractArray; dc=false)
  ...
  if dc
    ...
    @mtensor int2[k,l,c,d] := oovv[k,l,c,d] - oovv[l,k,c,d]
  end
  oovv = nothing
  @mtensor vT_voov[a,m,i,e] += T2a[a,d,i,l] * int2[l,m,d,e]
  ...
  oovv = nothing
  @mtensor vT_VOOV[A,M,I,E] += T2b[A,D,I,L] * int2[L,M,D,E]
  ...
  @mtensor vT_vOoV[a,M,i,E] += T2a[d,a,l,i] * int2[l,M,d,E]
...
function calc_correlation_norm(EC::ECInfo, U1, U2)
  T2 = load4idx(EC, "T_vvoo")
  Norm = 0.0
  @mtensor Norm += U2[a,b,i,j] * T2[a,b,i,j]
  if length(U1) > 0
    T1 = load2idx(EC, "T_vo")
    if length(T1) > 0
      @mtensor Norm += U2[a,b,i,j] * T1[a,i] * T1[b,j]
      @mtensor Norm += U1[a,i] * T1[a,i]
    end
  end
  return Norm
end
...
function calc_correlation_norm(EC::ECInfo, U1a, U1b, U2a, U2b, U2ab)
  T2 = load4idx(EC, "T_vvoo")
  Norm = 0.0
  @mtensor Norm += 0.25*(U2a[a,b,i,j] * T2[a,b,i,j])
  T2 = load4idx(EC, "T_VVOO")
  @mtensor Norm += 0.25*(U2b[a,b,i,j] * T2[a,b,i,j])
  T2 = load4idx(EC, "T_vVoO")
  @mtensor Norm += U2ab[a,b,i,j] * T2[a,b,i,j]
  if length(U1a) > 0 || length(U1b) > 0
    T1a = load2idx(EC, "T_vo")
    T1b = load2idx(EC, "T_VO")
    if length(T1a) > 0
      @mtensor Norm += 0.5*(U2a[a,b,i,j] * T1a[a,i] * T1a[b,j])
      @mtensor Norm += U1a[a,i] * T1a[a,i]
    end
    if length(T1b) > 0
      @mtensor Norm += 0.5*(U2b[a,b,i,j] * T1b[a,i] * T1b[b,j])
      @mtensor Norm += U1b[a,i] * T1b[a,i]
    end
    if length(T1a) > 0 && length(T1b) > 0
      @mtensor Norm += U2ab[a,b,i,j] * T1a[a,i] * T1b[b,j]
    end
  end
  return Norm
end
