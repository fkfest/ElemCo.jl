...
d_oovo = nothing
oovv = ints2(EC,"oovv")
@tensoropt (a=>10*x,b=>10*x,j=>x,i=>x,l=>x,k=>x,c=>10*x,d=>10*x) R2[a,b,j,i] += oovv[l,k,c,d] * T2[a,c,k,i] * T2[d,b,j,l]
@tensoropt (a=>10*x,b=>10*x,i=>x,j=>x,l=>x,k=>x,c=>10*x,d=>10*x) begin
X[a,b,i,j] := oovv[l,k,c,d] * T2[a,c,k,i] * T2[d,b,l,j]
R2[a,b,i,j] += X[a,b,i,j]
R2[b,a,j,i] += X[a,b,i,j]
end
@tensoropt (a=>10*x,b=>10*x,i=>x,j=>x,l=>x,k=>x,d=>10*x,c=>10*x) R2[a,b,i,j] += oovv[l,k,d,c] * T2[a,b,k,l] * T2[c,d,i,j]
...
@tensoropt (a=>10*x,b=>10*x,i=>x,j=>x,l=>x,k=>x,c=>10*x,d=>10*x) R2[a,b,i,j] -= 2 * oovv[l,k,c,d] * T2[c,a,k,i] * T2[d,b,l,j]
@tensoropt (a=>10*x,b=>10*x,i=>x,j=>x,l=>x,k=>x,d=>10*x,c=>10*x) begin
