...
"""
const ELEMENTS::Dict{String,Tuple{Int,Float64,String,String,String,String}} = Dict(
...
"""
const SUBSHELLS_NAMES = "spdfghiklm"

const SUBSHELL2L = Dict('s'=>0,'p'=>1,'d'=>2,'f'=>3,'g'=>4,'h'=>5,'i'=>6,'k'=>7,'l'=>8,'m'=>9)
...
function parse_electron_configuration(e::AbstractString)
  ...
  econf = e
  while econf[1] == '['
    end_core = findfirst(']', econf)
    if isnothing(end_core)
      error("missing ']' in the electron configuration string")
    end
    el_core = econf[2:end_core-1]
