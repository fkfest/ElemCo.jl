...
function dfhf(EC::ECInfo)
  ...
  save!(EC, EC.options.wf.eps, ϵ, description="DFHF orbital energies")
  occupation = append!(2*ones(Int, length(SP['o'])), zeros(Int, length(SP['v'])))
  save!(EC, EC.options.wf.occ, occupation, description="DFHF occuptations")
...
function dfhf_positron(EC::ECInfo)
  ...
  occupation = append!(2*ones(Int, length(SP['o'])), zeros(Int, length(SP['v'])))
  save!(EC, EC.options.wf.occ, occupation, description="DFHF occuptations")
  occupation_pos = append!(ones(Int, 1), zeros(Int, length(SP['v'])+1))
  save!(EC, EC.options.wf.occ_pos, occupation_pos, description="DFHF positron occuptations")
  save!(EC, EC.options.wf.orb, cMO, description="DFHF orbitals")
...
function dfuhf(EC::ECInfo)
  ...
  delete_temporary_files!(EC)
  save!(EC, EC.options.wf.eps, ϵ..., description="DFHF orbital energies")
  save!(EC, EC.options.wf.occ, [SP['o'], SP['O']]..., description="DFHF occuptations")
