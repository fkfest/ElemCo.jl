...
function dfhf_positron(EC::ECInfo)
  ...
  for it=1:EC.options.scf.maxit
    ...
    @tensoropt E_el = eden[p,q] * hsmall[p,q]
    @tensoropt E_pos = pden[p,q] * hsmall_pos[p,q]
    @tensoropt E_el += 0.5 * J[p,q] * eden[p,q]
    @tensoropt E_pos -= pden[p,q] * J[p,q]
    @tensoropt E_el -= 0.25 * eden[p,q] * K[p,q]
    EHF = E_el + E_pos + Enuc
    ΔE = EHF - previousEHF
    previousEHF = EHF
