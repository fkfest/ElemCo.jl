...
function _project_virtual_targets(cMO_virt::AbstractMatrix{Tc}, S::AbstractMatrix,
  ...
  M = select_lowdin_orth(metric; relthr=relthr)
  nsel = size(M, 2)
...
function _fragment_complement_virtual_rotation(cMO_virt::AbstractMatrix{T}, S::AbstractMatrix,
  ...
  C_complement, R_complement = _project_virtual_targets(cMO_virt, S, C_iao_target; relthr=relthr)

  C_ao_target = _support_ao_targets(cMO_virt, ao_atoms, support_atoms)
  _, R_aug = _project_virtual_targets(cMO_virt, S, C_ao_target; existing=C_complement, relthr=relthr)
